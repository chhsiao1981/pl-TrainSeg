...
class Unet_network:
    ...
    def symmetric_focal_tversky_loss(self, y_true, y_pred):
        from identify_axis import identify_axis 
        from keras import backend as K
        '''This is the implementation for binary segmentation.
        Parameters
        ----------
        y_true : tensor
            Tensor representing true labels.
        y_pred : tensor
            Tensor representing predicted labels.
        '''
        SMOOTH = 0.000001
        delta=0.7
        gamma=0.75
        ...
        print(y_pred)
        #axis=K.int_shape(y_pred)[-1]
        #axis = identify_axis(y_true.get_shape())
        # Calculate true positives (tp), false negatives (fn) and false positives (fp)     
        tp = K.sum(y_true * y_pred, axis=[1,2])
        fn = K.sum(y_true * (1 - y_pred), axis=[1,2])
        fp = K.sum((1 - y_true) * y_pred, axis=[1,2])
        dice_class = (tp + SMOOTH) / (tp + 0.7 * fn + 0.3 * fp + SMOOTH)

        # calculate losses separately for each class, enhancing both classes
        back_dice = (1 - dice_class[:, 0]) * K.pow(1 - dice_class[:, 0], 0.75)
        if y_true.shape[-1] == 4:
            fore_dice1 = (1 - dice_class[:, 1]) * K.pow(1 - dice_class[:, 1], 0.75)
            fore_dice2 = (1 - dice_class[:, 2]) * K.pow(1 - dice_class[:, 2], 0.75)
            fore_dice3 = (1 - dice_class[:, 3]) * K.pow(1 - dice_class[:, 3], 0.75)
            # Average class scores
            tmp = K.stack([back_dice, fore_dice1, fore_dice2, fore_dice3], axis=-1)
            print('tmp base symmetric tversky: ', tmp.shape)
            loss = K.mean(tmp)
            print('Check values:', tp, fn, fp, dice_class, back_dice, loss)
            return loss
